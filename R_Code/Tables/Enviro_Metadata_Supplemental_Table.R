...
meta.dat <- read_csv(meta.data.file) %>%
  filter(!str_detect(KinExp_ID, "UKG2"))

meta.dat.table <- meta.dat %>%
  select(KinExp_ID, Compound, Lat, Long, Region, Exp_Time_Local, Exp_Date_Local,
         Exp_Time_UTC, Exp_Date_UTC, sst, sss, chla, pc, pn, N_N, pp_13c, pp_14c) %>%
...
##export:
write_csv(meta.dat.table, file = "Tables/Meta_Data_Table.csv")
