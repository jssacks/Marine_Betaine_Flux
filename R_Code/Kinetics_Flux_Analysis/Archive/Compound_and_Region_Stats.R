...
kt.conc.dat <- kin.tot.metab.dat %>%
  mutate(D_norm = (mean_ks - Mean.Diss.Conc.nM)/(mean_ks)*100,
         error_D_norm = sqrt(((sqrt(SD.Diss.Conc.nM)^2 + (sd_ks)^2)/(mean_ks-Mean.Diss.Conc.nM))^2 + (sd_ks/mean_ks)^2),
         perc_diss_pool = (Mean.Diss.Conc.nM/metab.tot.nM)*100,
         error_perc_diss_pool = sqrt((metab.tot.nM.uncert/metab.tot.nM)^2 + (SD.Diss.Conc.nM/Mean.Diss.Conc.nM)^2)) %>%
  select(Cruise, exp, Compound, D_norm, error_D_norm, perc_diss_pool, error_perc_diss_pool)
