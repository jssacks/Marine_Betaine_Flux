...
####Make table of Dnorm values
dnorm.table <- read_csv(dnorm.file) %>%
  mutate(mean_ks = print(formatC(signif(mean_ks, digits=3), digits=3,format="fg")),
         Mean.Diss.Conc.nM = print(formatC(signif(Mean.Diss.Conc.nM, digits=3), digits=3,format="fg")),
         D_norm = print(formatC(signif(D_norm, digits=3), digits=3,format="fg")),
         D_norm_max = print(formatC(signif(D_norm_max, digits=3), digits=3,format="fg")),
         D_norm_min = print(formatC(signif(D_norm_min, digits=3), digits=3,format="fg"))) %>%
  rename("Experiment" = exp,
         "Kt (nM)" = mean_ks,
         "Dissolved Concentration (nM)" = Mean.Diss.Conc.nM,
         "Dnorm (%)" = D_norm,
         "Max Dnorm (%)" = D_norm_max,
         "Min Dnorm (%)" = D_norm_min)

#export:
write_csv(dnorm.table, file = "Tables/Flux_Tables/Dnorm_Table.csv")
