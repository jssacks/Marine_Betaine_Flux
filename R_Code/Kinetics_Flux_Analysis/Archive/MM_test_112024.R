...
####_____________________Rerun nls fitting for MM model with no explicit Sn included 


###
MM.nls.out.no.Sn <- data.frame(
  experiment = as.character(),
  parameter = as.character(),
  Estimate = as.numeric(),
  sigma = as.numeric(),
...
for (i in dat.exp.names$exp.nls) {
  x <- fit_nls_no_Sn(dat.MM.nls, mod.start.estimates.MM.nls, i)
  MM.nls.out.no.Sn <- rbind(MM.nls.out.no.Sn, x)
}

test.output.no.Sn <- MM.nls.out.no.Sn

mc.test.out.sum.no.Sn <- test.output.no.Sn %>%
  separate(experiment, into = c("Cruise", "exp", "text", "model"), remove = FALSE) %>%
  select(experiment, Cruise, exp, model, sigma, cor, iterations) %>%
  unique() %>%
  group_by(Cruise, exp) %>%
...
mc.test.remove.no.Sn <- mc.test.out.sum.no.Sn %>%
  filter(!sig.flag.high == TRUE,
         !sig.flag.low == TRUE,
         !cor.flag.high == TRUE,
         !cor.flag.low == TRUE,
...
mc.test.keep.no.Sn <- test.output.no.Sn %>%
  filter(experiment %in% mc.test.remove$experiment) %>%
  pivot_wider(names_from = parameter, values_from = Estimate) %>%
  separate(experiment, into = c("Cruise", "exp", "text", "model"), remove = FALSE) %>%
  group_by(Cruise, exp) %>%
...
for (i in dat.exp.names$exp.nls) {
  x <- fit_nls_new(dat.MM.nls, mod.start.estimates.MM.nls, i)
  MM.nls.out.new <- rbind(MM.nls.out.new, x)
}

test.output.new <- MM.nls.out.new

mc.test.out.sum.new <- test.output.new %>%
  separate(experiment, into = c("Cruise", "exp", "text", "model"), remove = FALSE) %>%
  select(experiment, Cruise, exp, model, sigma, cor, iterations) %>%
  unique() %>%
  group_by(Cruise, exp) %>%
...
mc.test.remove.new <- mc.test.out.sum.new %>%
  filter(!sig.flag.high == TRUE,
         !sig.flag.low == TRUE,
         !cor.flag.high == TRUE,
         !cor.flag.low == TRUE,
...
mc.test.keep.new <- test.output.new %>%
  filter(experiment %in% mc.test.remove$experiment) %>%
  pivot_wider(names_from = parameter, values_from = Estimate) %>%
  separate(experiment, into = c("Cruise", "exp", "text", "model"), remove = FALSE) %>%
  group_by(Cruise, exp) %>%
  reframe(mean_ks = mean(Ks),
          ...
          mean_s = mean(s_val),
          sd_s = sd(s_val),
          D_norm = ((mean_ks - mean_s)/mean_ks)*100)
