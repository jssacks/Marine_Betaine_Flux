...
write_csv(dat.smp, file = "Intermediates/TQS_dat.csv")
...
dat.std.hom <- rbind(dat.1, dat.2, dat.3, dat.4, dat.6) %>%
  ...
  mutate(SampID = str_replace(SampID, "Field2", "RC078_UKH1_2")) %>% # add in cruise info for RC078 UKH1
...
#save sample meta data for particulate quantification:
write_csv(mdat.raw, file = "Intermediates/Kinetics_Metadata_for_Part_Quant.csv")


#####______________ 5. Calculate incubation times from metadata to determine rates
mdat.tidy <- mdat.raw %>%
...
#####___________________ 6. Quantification__________________________________
...
write_csv(dat.blks.final, file = "Intermediates/Kinetics_Blanks_Final.csv")
...
####____ 7. Adjust quantification using blanks to get just microbial uptake values__________ 
...
####______________ 8. Calculate Uptake Rates____________________
dat.q.m <- dat.q.2 %>%
