...
fit_nls <- function(dat.input.rate, dat.input.pred, exp_name) {
  ...
  nls.out <- nls(y~(b*(x))/(a+x+s),start=list(a=a_start,b=b_start),
...
# 
# 
# ####function to fit nls equation to uptake kinetic curves without defining dissolved concentration
# fit_nls_no_Sn <- function(dat.input.rate, dat.input.pred, exp_name) {
#   
#   #pull out uptake rate data and starting predictions for the experiment defined in "exp_name"
#   dat.exp <- dat.input.rate %>%
#     filter(exp == exp_name)
...
#            cor = cor(y, predict(nls.out)),
#            iterations = nls.out$convInfo$finIter) %>%
#     select(experiment, parameter, Estimate, sigma, cor, iterations)
# }
# 
# 
# 
...
# 
# ####function to fit nls equation to uptake kinetic curves
# fit_nls_new <- function(dat.input.rate, dat.input.pred, exp_name) {
#   
#   #pull out uptake rate data and starting predictions for the experiment defined in "exp_name"
#   dat.exp <- dat.input.rate %>%
#     filter(exp == exp_name)
