...
plot.a <- ggplot(all.dat.hom, aes(x = Mean.Diss.Conc.nM, y = mean_ks)) +
  ...
  geom_point(aes(shape = factor(Compound, levels = c("GBT", "Homarine")), fill = factor(Region, levels = c("NPSG", "Eq", "PS", "NPTZ"))), size = 3.5) +
...
plot.b <- ggplot(all.dat.gbt, aes(x = Mean.Diss.Conc.nM, y = mean_ks)) +
  ...
  annotate("text", x = 1.1, y = 400, 
           label = "p = 0.12",
           size = 4)  +
  annotate("text", x = 1.1, y = 200, 
           label = expression(R^2~"="~0.65),
           size = 4) 
...
plot.c <- ggplot(all.dat, aes(x = perc_diss_pool, y = kt_s_ratio)) + 
  geom_smooth(alpha = 0.2, method = "lm", color = "gray20") +
  geom_errorbar(aes(ymin = kt_s_ratio_error_fig_min, ymax = kt_s_ratio_error_fig_max), width = 0.02) +
  geom_errorbarh(aes(xmin = error_perc_diss_pool_fig_min, xmax = error_perc_diss_pool_fig_max), height = 0.05) +
  geom_point(aes(shape = Compound, fill = Region), size = 3.5) +
...
#  ylab("Normalized Difference (% of Kt)") +
  annotate("text", x = 20, y = 1400, 
           label = "p < 0.0001",
           size = 4) +
  annotate("text", x = 20, y = 600, 
           label = expression(R^2~"="~0.85),
           size = 4) +
  scale_y_log10() +
  scale_x_log10()
...
ggsave(plot.all, filename = "Figures/MainTextFigure3_Kin_Flux_Conc_Relationships.png",
