...
#kinetics data
kin.file <- "Intermediates/NLS_Kin_Flux_vals.csv"
...
kin.curve.dat <- kin.dat %>%
  select(Cruise, exp, mean_ks, mean_vmax, Mean.Diss.Conc.nM) %>% 
  rename(mean_s = Mean.Diss.Conc.nM) %>%
  unite(c("Cruise", "exp"), col = "Cruise_Exp", remove = FALSE) %>%
  mutate(join.val = "x") %>%
  left_join(., spike.vals, relationship = "many-to-many") %>%
  mutate(nM_per_hour = mean_vmax*(treatment_nM)/(mean_ks+treatment_nM+mean_s)) %>%
  ...
  mutate(Region = case_when(str_detect(Cruise_Exp, "RC104") ~ "PS",
                            str_detect(Cruise_Exp, "RC078") ~ "PS",
                            ...
                            str_detect(Cruise_Exp, "TN397_UKH3") ~ "Eq",
                            str_detect(Cruise_Exp, "TN397_UKH4") ~ "Eq",
                            str_detect(Cruise_Exp, "TN397_UKH5") ~ "Eq")) %>%
...
###Plot kinetics Curves with inset plot:
...
kin.plot.B  <- ggplot(k.d.b, aes(x = treatment_nM, y = nM_per_hour_noS)) +
  geom_line(size = 1, alpha = 0.8, color = "#00A087FF") +
  theme_test() +
...
dat.fig <- read_csv(comp.file) %>%
  mutate(treatment = case_when(treatment == "Gluc" ~ "Glucose",
                               treatment == "Con" ~ "Control",
                               treatment == "Hom" ~ "Homarine",
                               treatment == "Trig" ~ "Trigonelline",
                               TRUE ~ treatment))
...
ggsave(exp.fig, filename = "Figures/MainTextFigure2_Kinetics_and_Competition_Experiments.png",
