...
#define inputs 
env.file <- "Intermediates/Dissolved_Metab_Environment_Summary.csv"
...
plot.a <- ggplot(env.dat.ord, aes(x = reorder(Cruise_Exp, -Mean.Diss.Conc.nM), y = Mean.Diss.Conc.nM, fill = reorder(Compound, order.comp))) +
  geom_col(color = "black", linewidth = 0.05, width = 0.7) +
  scale_fill_npg() +
...
plot.b <- ggplot(env.dat.ord, aes(x = reorder(Cruise_Exp, -Mean.Diss.Conc.nM), y = Mean.Diss.Conc.nM, fill = reorder(Compound, order.comp))) +
  ...
  theme_test() +
  xlab("Experiment") +
  ylab("Relative Concentration (Mole Fraction)") +
  labs(fill = "Compound") +
  theme(axis.text.x = element_text(angle = 45, vjust = 0.8, hjust = 0.8)) 
plot.b
...
ggsave(plot.ab, filename = "Figures/SupFig3_Environmental_Dissolved_Metabolomes.png",
